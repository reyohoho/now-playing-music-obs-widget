...
function makeToken() {
  ...
  return `nph-vk-${bytes[0].toString(36)}-${bytes[1].toString(36)}`;
}
...
export function createVkBridge(context) {
  ...
  const injectBridgeScript = () => {
    ...
    script.src = chrome.runtime.getURL("src/content/vkPageBridge.js");
    script.dataset.nphVkToken = token;
    script.async = false;
  ...
  const sendRequest = (request) => {
    ...
    return new Promise((resolve) => {
      const timer = setTimeout(() => {
        ...
        pending.delete(id);
        resolve({ ok: false, message: "vk bridge timeout" });
      }, 1200);
